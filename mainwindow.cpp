...
bool MainWindow::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if (obj == ui->Chu_Xiang1)
    {
        if (event->type() == QEvent::MouseButtonPress)
        {
            chButton = ui->Chu_Xiang1;
            ifPressed = true;
            chButton->raise();
            if (!CheckRoundCorrectAndSetRound())
            {
    ...
    if (obj == ui->Chu_Xiang2)
    {
        if (event->type() == QEvent::MouseButtonPress)
        {
            chButton = ui->Chu_Xiang2;
            ifPressed = true;
            chButton->raise();
            if (!CheckRoundCorrectAndSetRound())
            {
    ...
    if (obj == ui->Han_Xiang1)
    {
        if (event->type() == QEvent::MouseButtonPress)
        {
            chButton = ui->Han_Xiang1;
            ifPressed = true;
            chButton->raise();
            if (!CheckRoundCorrectAndSetRound())
            {
    ...
    if (obj == ui->Han_Xiang2)
    {
        if (event->type() == QEvent::MouseButtonPress)
        {
            chButton = ui->Han_Xiang2;
            ifPressed = true;
            chButton->raise();
            if (!CheckRoundCorrectAndSetRound())
            {
...
void MainWindow::on_Save_clicked()
{
    ...
    savedFileName += ".can";
    std::ofstream fout;
    fout.open("./Saved/" + savedFileName.toStdString(), std::ios::out);
    if (!fout.is_open())
    {
        QMessageBox msgBox;
        msgBox.setText("ERROR!");
...
void MainWindow::on_Load_clicked()
{
    ...
    std::ifstream fin;
    fin.open(fileLoadName.toStdString(), std::ios::in);
    if (!fin.is_open())
    {
        QMessageBox msgBox;
        msgBox.setText("ERROR!");
